...
public class Shuffle {
    ...
    public static void main(String[] args) {
        ...
        shuffle.setShuffledCaseOutputPath(shuffle.getShuffledCaseOutputPath() + File.separator + "shuffledCase");
        ...
        try {
            geneNameFileReader = new BufferedReader(new FileReader(shuffle.getToBeShuffledGeneNameFilePath()));
        ...
        try {
            frequencyFileReader = new BufferedReader(new FileReader(shuffle.getToBeShuffledFrequencyFilePath()));
            String tempString = null;
    ...
    public static void usage() {
        ...
        String usageString = "\n\t";
        usageString += "Shuffle positive variants into case files follow frequencies you defined. You should give the path of a positive variants file, path of a frequency file, path of a positive gene names file. Please see the [options] below.";
        usageString += "\n\n";
     ...
     */
    public boolean shuffleAccordingToGeneName(String diseaseFilePath, List<String> diseaseGeneNameList, List<Double> concentrationList, int variantSampleNumber) {
        try {
            ...
            for (Double concentration : concentrationList) {
                ...
                System.out.println("length of variant list related to a certain disease:" + variantList.size());

                //check and create "concentration folder", such as "5%"
                StringBuilder concentrationPath = new StringBuilder(getShuffledCaseOutputPath()).append(File.separatorChar).append(getPercentFormat(concentration, 2, 0));
                ...
                for (int i = 0; i < shuffledCaseFileList.size(); i++) {
                    if (i < counts) {
                        StringBuilder caseFilePathItemShuffled = new StringBuilder(concentrationPath).append(File.separatorChar)
                                .append(shuffledCaseFileList.get(i).getName());
                        caseFilePathItemShuffled = new StringBuilder(fileCreate(caseFilePathItemShuffled.toString()));
                        BufferedReader caseFileBufferedReader = new BufferedReader(new FileReader(caseFolderPath.toString() + File.separatorChar + shuffledCaseFileList.get(i).getName()));
                        try {
     ...
     */
    public void copyCaseFilesToShuffledCaseFolder(String caseFolderPath, List<Double> concentrationList) {
        ...
        for (Double concentration : concentrationList) {
            StringBuilder concentrationPath = new StringBuilder().append(getShuffledCaseOutputPath())
                    .append(File.separatorChar).append(getPercentFormat(concentration, 2, 0));
            concentrationPath = new StringBuilder(dirCreate(concentrationPath.toString()));
            ...
            for (String caseFile : caseFileList) {
                if (!shuffledDiseaseSet.contains(caseFile)) {
                    ...
                    StringBuilder shuffledCaseFilePath = new StringBuilder(getCanonicalPath(objectiveFolderFile)).append(File.separatorChar).append(caseFile);
