...
public class Onekey {
  ...
  public static void main(String[] args) throws InterruptedException {
    ...
    System.out.println(
      "Pickup finished! Case and control are generated at " + onekey.getOutputPath() + "!");
    ...
    /** finished shuffling. */
    System.out.println(
      "Shuffle finished! ShuffledCase are generated at " + shuffle.getShuffledCaseOutputPath()
        + "!");
    TimeUnit.SECONDS.sleep(1);
    ...
    for (Double frequency : frequencyList) {
      score.getAverageScoreForVariants(
        score.getCaseFolderPath() + File.separator + getPercentFormat(frequency, 2, 0), "case",
        caseVariantAvgScoreOutputPath);
    }
    /** control. */
    ...
    for (int j = 0; j < frequencyList.size(); j++) {
      System.out.println(
        "\n------- Score Each Gene start! Now processing frequency " + getPercentFormat(
          frequencyList.get(j), 2, 0) + "!\n");
      // case
      score.scoreEachGene(
        caseVariantAvgScoreOutputPath + File.separator + getPercentFormat(frequencyList.get(j), 2,
          0), "case", score.getInheritanceModel(), caseGeneScoreOutputPath, 5);
    }
    // control
    score.scoreEachGene(controlVariantAvgScoreOutputPath, "control", score.getInheritanceModel(),
      controlGeneScoreOutputPath, 5);
    ...
    statistic.setCaseFolderPath(
      statistic.getCaseFolderPath() + File.separator + statistic.getInheritanceModel());
    dirJudge(statistic.getCaseFolderPath());
    statistic.setControlFolderPath(
      statistic.getControlFolderPath() + File.separator + statistic.getInheritanceModel());
    dirJudge(statistic.getControlFolderPath());
    /** create gene score matrix output folder path. */
    statistic.setCaseOutputPath(
      statistic.getCaseOutputPath() + File.separator + "geneScoreMatrix" + File.separator
        + statistic.getInheritanceModel());
    dirsCreate(statistic.getCaseOutputPath());
    statistic.setControlOutputPath(
      statistic.getControlOutputPath() + File.separator + "geneScoreMatrix" + File.separator
        + statistic.getInheritanceModel());
    dirsCreate(statistic.getControlOutputPath());
    /** get gene score matrix for case and control. */
    for (int j = 0; j < frequencyList.size(); j++) {
      //case
      statistic.getGeneScoreMatrixFromGeneScoreFiles(
        statistic.getCaseFolderPath() + File.separator + getPercentFormat(frequencyList.get(j), 2,
          0), "case", statistic.getInheritanceModel(), statistic.getCaseOutputPath());
    }
    //control
    ...
    for (int j = 0; j < frequencyList.size(); j++) {
      String caseGeneScoreMatrixPath = getCanonicalPath(
        statistic.getCaseOutputPath() + File.separator + getPercentFormat(frequencyList.get(j), 2,
          0) + File.separator + "case_" + statistic.getInheritanceModel() + ".geneScoreMatrix");
      String controlGeneScoreMatrixPath = getCanonicalPath(
        statistic.getControlOutputPath() + File.separator + "control_" + statistic
          .getInheritanceModel() + ".geneScoreMatrix");
